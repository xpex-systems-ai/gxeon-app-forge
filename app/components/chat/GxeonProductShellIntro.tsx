  ...
  return (
    <section id="intro" className="mx-auto w-full max-w-5xl px-4 pt-8 text-center sm:pt-10 lg:px-0">
      ...
      <div className="mb-5 inline-flex max-w-3xl items-center rounded-xl border border-[#d9a441]/30 bg-[#d9a441]/10 px-4 py-3 text-left text-xs text-bolt-elements-textSecondary">
        <span className="mr-2 shrink-0 rounded-full bg-[#d9a441] px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.18em] text-black">
          Founder Preview
        </span>
        Módulos comerciais em fundação manual-first. Integrações reais entrarão por fases com aprovação humana.
      </div>
      <div className="mb-5 grid grid-cols-1 gap-3 text-left sm:grid-cols-2 lg:grid-cols-4">
        {PRODUCT_OS_MODULES.map((module) => (
          ...
          >
            ...
            <div className="mt-2 text-[10px] uppercase tracking-[0.16em] text-[#d9a441]">Manual-first preview</div>
          </div>
        ))}
      </div>
      <p className="text-xs text-bolt-elements-textSecondary">
        Powered by GXEON Systems AI. Based on open-source technology from bolt.diy. Transformed by GXEON into a Digital
        Product Creation OS.
      </p>
    </section>
  );
