...
`ContentFactoryExport` wraps the normalized draft, generated content, visible context payload, prompt, Markdown, safety flags, and export timestamp. Safety flags state manual-first, no guaranteed income, no auto-posting, no external send, no social API execution, no email API execution, no WhatsApp API execution, and local-only draft.
